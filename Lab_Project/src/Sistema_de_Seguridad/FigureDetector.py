...
class FigureDetector:
    def __init__(self, picam: Picamera2):
        ...
        # Figure that is going to be detected
    ...
    def detect_shape(self, frame: np.ndarray) -> bool:
        """
        ...
        kernel = np.ones((5, 5), np.uint8) # Kernel's size 
    ...
    def draw_detected_shape(self, frame: np.ndarray):
        """
        Draws the detected figure on the top-left corner of the frame.
        ...
        if self.figure.get_figure_type() == "square":
            cv2.rectangle(frame, (x, y), (x + size, y + size), color, -1)
        
        elif self.figure.get_figure_type() == "triangle":
            points = np.array([
                [x + size//2, y],
                [x, y + size],
                [x + size, y + size]
        ...
        elif self.figure.get_figure_type() == "circle":
            center = (x + size//2, y + size//2)
            radius = size//2
            cv2.circle(frame, center, radius, color, -1)
        
        elif self.figure.get_figure_type() == "pentagon":
            radius = size//2
            center = (x + size//2, y + size//2)
            points = []
            for i in range(5):
...
if __name__ == "__main__":
    ...
    picam = initialize()
    detector = FigureDetector(picam)
    
    while True:
        frame = detector.picam.capture_array() 
